...
class chartproduit extends StatefulWidget {
  final Widget child;

  chartproduit({Key key, this.child}) : super(key: key);

  _HomePageState createState() => _HomePageState();
}

class _HomePageState extends State<chartproduit> {
    ...
    await http.get(url, headers: {"Content-Type": "application/json"}).then(
      ...
      for (int i = 0; i < parsedJson.length; i++) {
        ...
        await http.get(url, headers: {"Content-Type": "application/json"}).then(
          ...
          List<Sales> linesalesdata2 = [];

          List<Sales> linesalesdata3 = [];

          for (int i = 0; i < lrate.length; i++) {
            print(lrate[i].med.toString());
            ...
            if (difference < 24) {
              ...
              data1.add(new Pollution(
                  lrate[i].date.year, lrate[i].med, lrate[i].rate));
            } else {
              if (difference2 < 7) {
            ...
            //print(lrate[i].med.toString());
            if (lrate[i].med.toString() == "H") {
              print ("hello 1");
              linesalesdata.add(new Sales(lrate[i].date.day, lrate[i].rate));
            } else if (lrate[i].med.toString() == "G") {
              print ("hello 2");
              linesalesdata1.add(new Sales(lrate[i].date.day, lrate[i].rate));
            } else if (lrate[i].med.toString() == "HFC") {
              print ("hello 3");
              linesalesdata2.add(new Sales(lrate[i].date.day, lrate[i].rate));
            } else{
              print ("hello 4");
              linesalesdata3.add(new Sales(lrate[i].date.day, lrate[i].rate));
            }
          }
          if (data1.isNotEmpty) {
            _seriesData.add(
                ...
                child: Container(
                  child: Center(
                    child: Column(
                      children: <Widget>[
                        Text(
                          'Vote sur les produit pour un semaine :',
                          style: TextStyle(
                        ...
                        Expanded(
                          child: charts.LineChart(_seriesLineData,
                              ...
                              behaviors: [
                                new charts.ChartTitle('Jour :'+DateTime.now().month.toString()+'/'+DateTime.now().year.toString(),
                                    behaviorPosition:
...
class RateOfMed {
  ...
  RateOfMed(this.rate, this.med, this.date);
}
